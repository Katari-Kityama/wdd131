...
let foodContainer = document.querySelector('.replace-me');
...
function search() {
    ...
    let sortedRecipes = filteredrecipes.sort((a, b) => b.rating - a.rating);
...
function generateTags(recipe) {
  let recipeTags = recipe
  let tagsHTML = [];

  recipeTags.forEach(tag => {
    tagsHTML.push(`<button type="${tag}">${tag}</button>`);
  });

  return tagsHTML.join('');
}

function foodTemplate(recipe) {
  let star = "⭐";
  let star3 = "☆";
  let star4 = "☆";
  let star5 = "☆";
  ...
  <section class="dessert-area">
    ...
    <div>
        ...
        <span class="rating" role="img" aria-label="${recipe.rating} out of 5 stars">
            <span aria-hidden="true" class="icon-star">⭐</span>
            <span aria-hidden="true" class="icon-star">⭐</span>
            <span aria-hidden="true" class="icon-star">${star3}</span>
            <span aria-hidden="true" class="icon-star">${star4}</span>
            <span aria-hidden="true" class="icon-star-empty">${star5}</span>
        </span>
...
function renderMeals(recipe) {
  let html = foodTemplate(recipe);
  foodContainer.innerHTML += html
}

function init() {
  renderMeals(recipes[randomNum]);
}
